...
class UpdateBalanceView(LoginRequiredMixin, generic.View):
    def get(self, request, pk):
        ...
        combined_form = namedtuple('combined_form',
                                   ['uniqueid', 'form_balance', 'form_cheque', 'chequeid', 'account_str', 'bank_str',
                                    'balance_id', 'is_active'])
        for i in balancesheet:
            ...
            combined_forms.append(
                combined_form(i.id, balance_forms, cheque_forms, chequeid, account_str, bank_str, balance_id,
                              is_active))
        context = {
    ...
    def post(self, request, pk):
        ...
        all_debt = 0
        user = request.user
        balancesheets = BalanceSheet.objects.filter(user=request.user, document=document)
        combinedform = namedtuple('combined_form', ['uniqueid', 'form_balance', 'form_cheque'])
        combined_forms = []
        all_valid = True
        for i in balancesheets:
            if i.is_active == False:
                if i.transaction_type == "debt":
                    all_debt += i.amount or 0
                elif i.transaction_type == "credit":
                    all_credit += i.amount or 0
            if i.id in deleted_ids and i.is_active:
                i.delete()
                if i.cheque:
                    if i.cheque.balance_sheet.filter(is_active=False).last():
                        status = i.cheque.balance_sheet.filter(is_active=False).last()
...
class BalanceListView(LoginRequiredMixin, generic.View):
    def get(self, request):
        qs = BalanceSheet.objects.filter(user=request.user.id).select_related('cheque').order_by(
...
class ChangeStatusCheque(generic.View):
    def get(self, request, pk):
        ...
        if balancesheet.transaction_type == 'debt':
            ...
            balance_forms = BalanceSheetForm(prefix=f"old-update-cheque-update-balance",
                                             initial={"transaction_type": 'debt', 'amount': balancesheet.amount})
